...
class check():
    ...
    def restricted(self, string):
        ...
        convert = conversion()
        string = convert.numbersToLetters(string)
        self.restrictedWords = open('restrictedWords.txt').read().splitlines()
        for i in range(len(self.restrictedWords)):
            if self.restrictedWords[i] in string:
                return True
        return False
...
def writeFile(string):
    ...
    if string not in registeredPlates.read().splitlines():
        registeredPlates.write(string + '\n')
    registeredPlates.close()
    
def clearFile():
    file = open('registeredPlates.txt', 'w')
    file.write('')
    file.close()
